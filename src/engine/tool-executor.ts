...
export class ToolExecutor {
    ...
    static getToolDescriptions(): string {
        return [
            ...
            '```',
            '',
            '### browser — 控制浏览器 (Playwright)',
            '用法示例：',
            '```',
            '# 打开网页',
            '<tool_call>{"tool":"browser","args":{"action":"goto","url":"https://example.com"}}</tool_call>',
            '',
            '# 点击按钮（用 CSS 选择器或 text 选择器）',
            '<tool_call>{"tool":"browser","args":{"action":"click","selector":"button:text(\'Sign in\')" }}</tool_call>',
            '',
            '# 输入文字',
            '<tool_call>{"tool":"browser","args":{"action":"type","selector":"input[name=email]","text":"user@example.com"}}</tool_call>',
            '',
            '# 读取页面内容（可见文字 + 交互元素列表）',
            '<tool_call>{"tool":"browser","args":{"action":"content"}}</tool_call>',
            '',
            '# 等待元素出现',
            '<tool_call>{"tool":"browser","args":{"action":"wait","selector":".api-key"}}</tool_call>',
            '',
            '# 滚动页面',
            '<tool_call>{"tool":"browser","args":{"action":"scroll","direction":"down"}}</tool_call>',
            '',
            '# 关闭浏览器',
            '<tool_call>{"tool":"browser","args":{"action":"close"}}</tool_call>',
            '```',
            '',
            '**规则：**',
